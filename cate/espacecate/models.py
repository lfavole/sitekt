...
class Child(CommonChild):
    ...
    annees_evf = models.fields.IntegerField(
        "Années d'éveil à la foi", validators=[MinValueValidator(0), MaxValueValidator(4)]
    )
    ...
    annee_pardon = models.fields.IntegerField(
        "Année du Sacrement du Pardon", blank=True, null=True, validators=[MaxValueValidator(get_current_year)]
    )
    ...
    fieldsets = [
        ...
        ("École", {"fields": ("ecole", "classe", "redoublement")}),
        (
            "Caté",
            {
                "fields": (
